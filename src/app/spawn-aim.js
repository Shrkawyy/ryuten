  ...
  rp.modules.installSpawnAim=(h,r)=>{
    ...
    const aim=rp.spawnAim={
      ...
      handleMouse(event,type){
        ...
        if(type==='mouseup'&&consumed){consumed=false;return true;}
        ...
        consumed=m.setSpawnTarget(aim.mouseWorld(event));return consumed;
      },
      frame(){
        const point=m.intent?.requestedAnchor||m.spawnTarget;
        const visible=!!(m.multi&&m.mouseSpawn&&point&&!r.rs._4020&&(!point.endpoint||point.endpoint===h.network.url));
