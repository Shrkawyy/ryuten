# Hold-to-preview target — 1.2.1-xplus.10

Hold right-click to show and move the target square. Releasing right-click hides it immediately; the selected world point remains available for Tab/spawn. The square also hides after spawn is sent, when both slots are alive, or on focus loss. Mouse spawn and background loading are retained.

Install `dist/RYUTEN-Senpa.user.js` and disable the older version. Current verification is in `verification/hold-target-tests.txt`; older release reports below are historical.
